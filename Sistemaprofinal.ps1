...
function Executar-Instalacao {
    ...
    try {
        ...
        Write-Host "ERRO: Falha no download. Verifique sua conexao." -ForegroundColor Red
        Pause; return
    }
    ...
    try {
        # Lógica para extrair .RAR (Tenta WinRAR, depois 7-Zip)
        if (Test-Path "C:\Program Files\WinRAR\WinRAR.exe") {
            ...
            Write-Host "AVISO: WinRAR ou 7-Zip nao detectados. Tentando extracao nativa..." -ForegroundColor Yellow
        ...
        # Copia dos arquivos para a Steam (Procura pastas extraidas)
        $folderConfig = Get-ChildItem -Path $tmp -Filter "Config" -Recurse | Select-Object -First 1
        if ($folderConfig) {
        ...
        Write-Host "ERRO na extracao: O arquivo baixado pode estar corrompido." -ForegroundColor Red
    }
}
